...
# CRITICAL: ALL PowerShell files must be listed here. Adding a new file? Add it here immediately!
$psFiles = @(
    "MiracleBoot.ps1",
...
function Test-RuntimeModuleLoad {
    Write-Log "Testing runtime module loading..." ([ConsoleColor]::Yellow)
    
    $modules = @(
        @{ Name = "WinRepairCore.ps1"; Path = "Helper\WinRepairCore.ps1"; Functions = @("Get-WindowsVolumes", "Get-EnvironmentType") },
        @{ Name = "NetworkDiagnostics.ps1"; Path = "Helper\NetworkDiagnostics.ps1"; Functions = @("Get-NetworkAdapterStatus", "Test-NetworkConnectivity") },
    ...
    foreach ($module in $modules) {
        $fullPath = Join-Path $root $module.Path
        Write-Log "  Testing: $($module.Name)..." ([ConsoleColor]::Gray)
        
        if (-not (Test-Path $fullPath)) {
            Write-Log "    [FAIL] File not found: $fullPath" ([ConsoleColor]::Red)
            $global:overallExitCode = 1
            $allPassed = $false
            continue
        }
        ...
        try {
            ...
            if ($Error.Count -gt 0) {
                ...
                Write-Log "    [FAIL] Errors during load: $errorMsg" ([ConsoleColor]::Red)
                $global:overallExitCode = 1
                $allPassed = $false
                continue
            }
            
            # Check if expected functions exist (if any specified)
            if ($module.Functions.Count -gt 0) {
                ...
                if ($missingFunctions.Count -gt 0) {
                    Write-Log "    [FAIL] Missing functions: $($missingFunctions -join ', ')" ([ConsoleColor]::Red)
                    $global:overallExitCode = 1
                    $allPassed = $false
                } else {
                    Write-Log "    [PASS] Module loaded successfully" ([ConsoleColor]::Green)
                }
            } else {
                # No functions to check - just verify it loaded without errors
                Write-Log "    [PASS] Module loaded successfully" ([ConsoleColor]::Green)
            }
            
        } catch {
            Write-Log "    [FAIL] Exception: $($_.Exception.Message)" ([ConsoleColor]::Red)
            Write-Log "    Line: $($_.InvocationInfo.ScriptLineNumber)" ([ConsoleColor]::Yellow)
            $global:overallExitCode = 1
            $allPassed = $false
        }
    }
...
try {
    ...
    # Load GUI module (this will catch syntax errors and LogAnalysis.ps1 loading)
    ...
    # Verify LogAnalysis functions are available (since WinRepairGUI.ps1 loads it)
    if (-not (Get-Command Get-ComprehensiveLogAnalysis -ErrorAction SilentlyContinue)) {
        Write-Host "[WARN] Get-ComprehensiveLogAnalysis not found (LogAnalysis.ps1 may not have loaded)" -ForegroundColor Yellow
    } else {
        Write-Host "[PASS] LogAnalysis.ps1 loaded successfully via WinRepairGUI.ps1" -ForegroundColor Green
    }
...
# PHASE 0.5: Runtime Module Loading Validation (Catches encoding/runtime issues)
Write-Host ""
Write-Host "========================================================================" -ForegroundColor Cyan
Write-Host "  PHASE 0.5: RUNTIME MODULE LOADING VALIDATION" -ForegroundColor Cyan
Write-Host "========================================================================" -ForegroundColor Cyan
Write-Host ""

$moduleLoadPassed = Test-RuntimeModuleLoad
if (-not $moduleLoadPassed) {
    Write-Host ""
    Write-Host "========================================================================" -ForegroundColor Red
    Write-Host "  RUNTIME MODULE LOADING FAILED - FIX ERRORS BEFORE PROCEEDING" -ForegroundColor Red
    Write-Host "========================================================================" -ForegroundColor Red
    Write-Host ""
    Write-Host "One or more modules failed to load at runtime. This may indicate:" -ForegroundColor Yellow
    Write-Host "  - Encoding issues (UTF-8 BOM problems)" -ForegroundColor Yellow
    Write-Host "  - Missing dependencies" -ForegroundColor Yellow
    Write-Host "  - Runtime syntax errors not caught by parser" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "Fix these errors and run SuperTest again." -ForegroundColor Yellow
    exit 1
} else {
    Write-Log "RUNTIME MODULE LOADING PASSED: All modules loaded successfully" ([ConsoleColor]::Green)
}
